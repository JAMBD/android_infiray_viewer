	...
	public Bitmap bitmapFromData(byte[] data){
		...
		for (int i=0; i<width; i++){
			for (int j=0; j<height; j++){
				...
				final int y = last_frame[(i*height + j) * 2] & 0xFF;
				final int v = last_frame[1 + (i*height + (j&0xFE)) * 2] & 0xFF;
				final int u = last_frame[1 + (i*height + (j&0xFE)+1) * 2] & 0xFF;
				int r = y + r_v[v];
				int g = y + g_v[v] + g_u[u];
				int b = y + b_u[u];
				if (r < 0) r = 0;
				...
				if (g > 255) g = 255;
				if (b > 255) b = 255;
				pixels[i*height + j] = 0xFF000000 | r | (g<<8) | (b << 16);
			}
		}
