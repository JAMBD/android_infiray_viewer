...
public class MainActivity extends Activity {
    ...
    private int minPixelRaw = 0,
        maxPixelRaw = 0;
    private int minPixelX = 0,
        minPixelY = 0;
    private int maxPixelX = 0,
        maxPixelY = 0;

    // ROI settings
    private boolean showROIOverlay = false;
    private int roiX1 = 64,
        roiY1 = 48; // Top-left in frame coords (default: center quarter)
    private int roiX2 = 192,
        roiY2 = 144; // Bottom-right in frame coords
    private int roiMinPixelRaw = 0,
        roiMaxPixelRaw = 0,
        roiAvgPixelRaw = 0;
    private int roiMinPixelX = 0,
        roiMinPixelY = 0;
    private int roiMaxPixelX = 0,
        roiMaxPixelY = 0;
    ...
    private void loadSettings() {
        ...
        includeOverlayInSaves = prefs.getBoolean(
            "includeOverlayInSaves",
            false
        );
    ...
    private final BroadcastReceiver usbReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (UsbManager.ACTION_USB_DEVICE_DETACHED.equals(action)) {
                Log.d(
                    "ThermalCamera",
                    "Device detached, setting native_stream=0"
                );
                native_stream = 0;
                Toast.makeText(
                    context,
                ...
                Log.d(
                    "ThermalCamera",
                    "Device attached, calling connectCamera"
                );
                if (connectCamera()) {
                    Toast.makeText(
                        context,
                ...
                if (
                    ...
                    Log.d(
                        "ThermalCamera",
                        "Permission granted, calling connectCamera"
                    );
                    if (connectCamera()) {
                        Toast.makeText(
                            context,
    ...
    private boolean connectCamera() {
        Log.d(
            "ThermalCamera",
            "connectCamera called, current native_stream=" +
                native_stream +
                ", fd=" +
                fd
        );
        ...
        if (native_stream != 0) {
            Log.d(
                "ThermalCamera",
                "Already have active stream, skipping connection"
            );
            return true;
        }
        UsbManager usbManager = (UsbManager) getSystemService(
            Context.USB_SERVICE
        );
        ...
        for (UsbDevice device : deviceList.values()) {
            Log.d(
                "ThermalCamera",
                "Device: " + device.getVendorId() + ":" + device.getProductId()
            );
            ...
            Log.d(
                "ThermalCamera",
                "initializeStream returned: " + native_stream
            );
            if (native_stream == 0) {
                ...
                Log.d(
                    "ThermalCamera",
                    "initializeStream SUCCESS - stream is " + native_stream
                );
                ...
                new Thread(() -> {
                    ...
                    Log.d(
                        "ThermalCamera",
                        "Calibration complete, displaying frames"
                    );
                })
                    .start();
            }
    ...
    public Bitmap bitmapARGBFromByte(byte[] data) {
        ...
        if (data.length < kExpectedSize) {
            Log.w(
                "ThermalCamera",
                "Short frame: " + data.length + " < " + kExpectedSize
            );
        ...
        if (mirrorImage) {
            ...
            for (int half = 0; half < 2; half++) {
                ...
                for (int y = 0; y < kFrameHeight / 2; y++) {
                    ...
                    int botOffset =
                        baseOffset + (kFrameHeight - 1 - y) * rowBytes;
        ...
        centerPixelRaw =
            (data[centerOffset] & 0xFF) |
            ((data[centerOffset + 1] & 0xFF) << 8);

        // Detect when frames have actually changed after a gain switch
        if (waitingForGainFrame) {
            boolean frameChanged =
                Math.abs(centerPixelRaw - preSwitchCenterRaw) > 100;
            boolean timedOut =
                (System.currentTimeMillis() - gainSwitchTimeMs) > 10000;
            if (frameChanged || timedOut) {
                ...
                runOnUiThread(() -> {
                    ...
                    Log.i(
                        "ThermalCamera",
                        "Gain mode: " +
                            (isLowGain ? "Low (wide)" : "High (narrow)") +
                            " (frame changed)"
                    );
                });
            }
        ...
        if (showROIOverlay) {
            int rx1 = Math.min(roiX1, roiX2),
                rx2 = Math.max(roiX1, roiX2);
            int ry1 = Math.min(roiY1, roiY2),
                ry2 = Math.max(roiY1, roiY2);
    ...
    public void startRecording(View view) {
        ...
        try {
            ...
            int bitrate = includeOverlayInSaves
                ? VIDEO_BITRATE_OVERLAY
                : VIDEO_BITRATE_RAW;
        ...
        public void run() {
            ...
            if (native_stream == 0) {
                if (frameCount % 100 == 0) {
                    Log.d(
                        "ThermalCamera",
                        "runnable: native_stream is 0, skipping frame grab"
                    );
                }
                frameCount++;
                return;
            ...
            if (last_frame == null) {
                Log.d(
                    "ThermalCamera",
                    "runnable: grabFrame returned null for stream " +
                        native_stream
                );
                return;
            }
            ...
            Bitmap thermalWithCrosshairs = bitmap.copy(
                Bitmap.Config.ARGB_8888,
                true
            );
            ...
            if (showMinMaxOverlay) {
                ...
                canvas.drawLine(
                    minScreenX,
                    minScreenY - miniCrosshairSize,
                    minScreenX,
                    minScreenY + miniCrosshairSize,
                    paint
                );
                canvas.drawLine(
                    maxScreenX - miniCrosshairSize,
                    maxScreenY,
                    maxScreenX + miniCrosshairSize,
                    maxScreenY,
                    paint
            ...
            if (showROIOverlay) {
                ...
                canvas.drawLine(
                    roiMinScreenX - miniCrosshairSize,
                    roiMinScreenY,
                    roiMinScreenX + miniCrosshairSize,
                    roiMinScreenY,
                    paint
                ...
                canvas.drawLine(
                    roiMinScreenX,
                    roiMinScreenY - miniCrosshairSize,
                    roiMinScreenX,
                    roiMinScreenY + miniCrosshairSize,
                    paint
                );
                canvas.drawLine(
                    roiMaxScreenX - miniCrosshairSize,
                    roiMaxScreenY,
                    roiMaxScreenX + miniCrosshairSize,
                    roiMaxScreenY,
                    paint
                ...
                canvas.drawLine(
                    roiMaxScreenX,
                    roiMaxScreenY - miniCrosshairSize,
                    roiMaxScreenX,
                    roiMaxScreenY + miniCrosshairSize,
                    paint
                );
            ...
            displayBitmapWithOverlays = Bitmap.createBitmap(
                thermalWidth,
                bannerHeight + thermalHeight,
                Bitmap.Config.ARGB_8888
            );
            ...
            if (isRecording) {
                ...
                if (includeOverlayInSaves) {
                    ...
                    videoBitmap = Bitmap.createScaledBitmap(
                        displayBitmapWithOverlays,
                        currentVideoWidth,
                        currentVideoHeight,
                        true
                    );
                    ...
                    videoBitmap = Bitmap.createBitmap(
                        rawBitmap,
                        0,
                        0,
                        rawBitmap.getWidth(),
            ...
            if (perfFrameCount >= 100) {
                Log.i(
                    "ThermalCamera",
                    String.format(
                        "PERF: avg frame=%.1fms bitmap=%.1fms encode=%.1fms (over %d frames)",
    ...
    protected void onResume() {
        ...
        if (native_stream == 0) {
            Log.d(
                "ThermalCamera",
                "onResume: native_stream is 0, trying to connect"
            );
            connectCamera();
        }
    }
    ...
    private float tempSeekBarToDegrees(int progress) {
        return (
            TEMP_ABS_MIN +
            ((TEMP_ABS_MAX - TEMP_ABS_MIN) * progress) /
            (float) TEMP_SEEKBAR_MAX
        );
    }

    private int degreesToTempSeekBar(float degrees) {
        return Math.round(
            ((degrees - TEMP_ABS_MIN) / (TEMP_ABS_MAX - TEMP_ABS_MIN)) *
                TEMP_SEEKBAR_MAX
        );
    }
    ...
    private int degreesToSeekBar(float degrees) {
        float t = (float) Math.sqrt(
            (degrees - RANGE_MIN) / (RANGE_MAX - RANGE_MIN)
        );
    ...
    private void initTopBottomFromCurrentImage() {
        ...
        // Clamp
        manualBottomDegrees = Math.max(
            TEMP_ABS_MIN,
            Math.min(TEMP_ABS_MAX, manualBottomDegrees)
        );
        manualTopDegrees = Math.max(
            TEMP_ABS_MIN,
            Math.min(TEMP_ABS_MAX, manualTopDegrees)
        );
    ...
    private void updateTopBottomUI() {
        ...
        if (topSeekBar != null) topSeekBar.setProgress(
            degreesToTempSeekBar(manualTopDegrees)
        );
        if (bottomSeekBar != null) bottomSeekBar.setProgress(
            degreesToTempSeekBar(manualBottomDegrees)
        );
        if (topLabel != null) topLabel.setText(
            String.format("%.1f", manualTopDegrees)
        );
        if (bottomLabel != null) bottomLabel.setText(
            String.format("%.1f", manualBottomDegrees)
        );
    }
    ...
    private void updateRangeOverlayVisibility() {
        ...
        if (overlay != null) {
            overlay.setVisibility(
                colorScaleMode == 1 ? View.VISIBLE : View.GONE
            );
        }
        View topBottomOverlay = findViewById(R.id.manualTopBottomOverlay);
        if (topBottomOverlay != null) {
            topBottomOverlay.setVisibility(
                colorScaleMode == 2 ? View.VISIBLE : View.GONE
            );
        }
    }
    ...
    private void showSettingsDialog() {
        ...
        new AlertDialog.Builder(this)
            ...
            .setPositiveButton("OK", (dialog, which) -> {
                ...
                Log.d(
                    "ThermalCamera",
                    "Settings updated: colorScaleMode=" +
                        colorScaleMode +
                        ", centerCrosshair=" +
                        showCenterCrosshair +
                        ", minMax=" +
    ...
    private void handleControlAction(String action) {
        ...
        switch (action) {
            ...
            case "capture_raw":
                ...
                SimpleDateFormat rawDateFormat = new SimpleDateFormat(
                    "yyyy_MM_dd_HH_mm_ss",
                    Locale.getDefault()
                );
                Date rawNow = new Date();
                String rawDateTimeString = rawDateFormat.format(rawNow);
                saveBytesToFileInDownloads(
                    getApplicationContext(),
                    last_frame,
                    "thermal_camera_" + rawDateTimeString + ".bin"
                );
                break;
            case "start_video":
                if (!isRecording) {
            ...
            case "toggle_center_crosshair":
                ...
                Log.i(
                    "ThermalCamera",
                    "Center crosshair: " + showCenterCrosshair
                );
                break;
            case "toggle_minmax":
            ...
            case "toggle_overlay_in_saves":
                ...
                Log.i(
                    "ThermalCamera",
                    "Include overlay in saves: " + includeOverlayInSaves
                );
                break;
            case "set_scale_auto":
                colorScaleMode = 0;
            ...
            case "set_scale_manual":
                ...
                Log.i(
                    "ThermalCamera",
                    "Color scale mode: Manual Center ±" +
                        manualRangeDegrees +
                        "°C"
                );
                break;
            case "set_scale_topbottom":
                ...
                Log.i(
                    "ThermalCamera",
                    "Color scale mode: Manual Top/Bottom " +
                        manualBottomDegrees +
                        "-" +
                        manualTopDegrees +
                        "°C"
                ...
                new Thread(() -> {
                    ...
                    if (result == 0) {
                        ...
                        runOnUiThread(() -> {
                            if (gb != null) {
                                gb.setText(
                                    isLowGain ? "Low Range" : "High Range"
                                );
                                gb.setEnabled(true);
                            }
                            isSwitchingGain = false;
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (UsbManager.ACTION_USB_DEVICE_ATTACHED.equals(intent.getAction())) {
            Log.d(
                "ThermalCamera",
                "Device attached via manifest intent, calling connectCamera"
            );
            if (connectCamera()) {
                Toast.makeText(
                    this,
        ...
        if (controlAction != null) {
            if (controlAction.equals("set_manual_range")) {
                ...
                if (range >= RANGE_MIN && range <= RANGE_MAX) {
                    ...
                    if (label != null) {
                        label.setText(
                            String.format("±%.1f", manualRangeDegrees)
                        );
                    }
                    saveSettings();
                    Log.i(
                        "ThermalCamera",
                        "Manual range set to ±" + manualRangeDegrees + "°C"
                    );
                } else {
                    Log.w(
                        "ThermalCamera",
                        "Invalid range value: " + range + " (must be 0.5-170.0)"
                    );
                }
                ...
                if (
                    ...
                    manualTopDegrees = Math.max(
                        temp,
                        manualBottomDegrees + 0.5f
                    );
                    updateTopBottomUI();
                    saveSettings();
                    Log.i(
                        "ThermalCamera",
                        "Manual top set to " + manualTopDegrees + "°C"
                    );
                } else {
                ...
                if (
                    ...
                    manualBottomDegrees = Math.min(
                        temp,
                        manualTopDegrees - 0.5f
                    );
                    updateTopBottomUI();
                    saveSettings();
                    Log.i(
                        "ThermalCamera",
                        "Manual bottom set to " + manualBottomDegrees + "°C"
                    );
                } else {
                    Log.w("ThermalCamera", "Invalid temp value: " + temp);
                }
    ...
    public static void saveBytesToFileInDownloads(
        Context context,
        byte[] data,
        String fileName
    ) {
        ...
        ); // or another more specific MIME type.
        contentValues.put(
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    saveBytesToFileInDownloads(
                        getApplicationContext(),
                        last_frame,
                        "thermal_camera_" + dateTimeString + ".bin"
                    );
                }
            }
        );
        ...
        if (thumb != null) thumb.setColorFilter(
            0xFFFF00FF,
            PorterDuff.Mode.SRC_IN
        );
        rangeSeekBar
            .getProgressDrawable()
            .setColorFilter(0xFFFF00FF, PorterDuff.Mode.SRC_IN);
        ...
        seekBarContainer.addOnLayoutChangeListener(
            (
                v,
                left,
                top,
        ...
        if (topThumb != null) topThumb.setColorFilter(
            0xFFFF4444,
            PorterDuff.Mode.SRC_IN
        );
        ...
        if (bottomThumb != null) bottomThumb.setColorFilter(
            0xFF4488FF,
            PorterDuff.Mode.SRC_IN
        );
        bottomTempSeekBar
            .getProgressDrawable()
            .setColorFilter(0xFF4488FF, PorterDuff.Mode.SRC_IN);
        ...
        topBottomContainer.addOnLayoutChangeListener(
            (
                v,
                left,
                top,
        ...
        bottomTempSeekBar.setOnSeekBarChangeListener(
            new SeekBar.OnSeekBarChangeListener() {
                @Override
                public void onProgressChanged(
                    SeekBar seekBar,
        ...
        touchInterceptor.setLayoutParams(
        ...
        touchInterceptor.setOnTouchListener((v, event) -> {
            ...
            int touchProgress = (int) (TEMP_SEEKBAR_MAX *
                (1.0f - y / containerHeight));
            touchProgress = Math.max(
                0,
                Math.min(TEMP_SEEKBAR_MAX, touchProgress)
            );

            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    // Pick the closer thumb on initial touch
                    int distToTop = Math.abs(
                        touchProgress - topTempSeekBar.getProgress()
                    );
                    int distToBottom = Math.abs(
                        touchProgress - bottomTempSeekBar.getProgress()
                    );
                    activeSeekBar[0] = (distToTop <= distToBottom)
                        ? topTempSeekBar
                        : bottomTempSeekBar;
        ...
        WindowManager windowManager = (WindowManager) this.getSystemService(
        ...
        imageView.setOnTouchListener((v, event) -> {
            ...
            int frameX = Math.min(
                255,
                Math.max(0, (int) (event.getY() / scale))
            );
            int frameY = Math.min(
                191,
                Math.max(0, (int) (191 - event.getX() / scale))
            );
